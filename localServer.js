...
function staticRespond(req){
	...
	exists(fileName).then(
		...
		(file) => {
			//设定返回文档的MIME类型
			let resHeaders = req.headers;
			resHeaders.set("Content-Type",req.url.match(/\.([^.]*)$/));
			let fileRes = new Response(file,{headers:resHeaders});//{ body: file };
